...
for k1 in range(int(np.ceil((n + 9) / 10)), n + 1):
    ks[0] = k1
    for k2 in range((n - sum(ks[0:1])) // 9, min(k1 - 1, n - sum(ks[0:1])) + 1):
        ks[1] = k2
        for k3 in range((n - sum(ks[0:2])) // 8, min(k1 - 1, n - sum(ks[0:2])) + 1):
            ks[2] = k3
            for k4 in range((n - sum(ks[0:3])) // 7, min(k1 - 1, n - sum(ks[0:3])) + 1):
                ks[3] = k4
                for k5 in range((n - sum(ks[0:4])) // 6, min(k1 - 1, n - sum(ks[0:4])) + 1):
                    ks[4] = k5
                    for k6 in range((n - sum(ks[0:5])) // 5, min(k1 - 1, n - sum(ks[0:5])) + 1):
                        ks[5] = k6
                        for k7 in range((n - sum(ks[0:6])) // 4, min(k1 - 1, n - sum(ks[0:6])) + 1):
                            ks[6] = k7
                            for k8 in range((n - sum(ks[0:7])) // 3, min(k1 - 1, n - sum(ks[0:7])) + 1):
                                ks[7] = k8
                                for k9 in range((n - sum(ks[0:8])) // 2, min(k1 - 1, n - sum(ks[0:8])) + 1):
                                    ks[8] = k9
                                    for k10 in range((n - sum(ks[0:9])) // 1, min(k1 - 1, n - sum(ks[0:9])) + 1):
                                        ks[9] = k10
